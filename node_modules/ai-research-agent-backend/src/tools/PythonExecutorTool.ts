...
export class PythonExecutorTool implements Tool {
  ...
  description = 'Execute Python code safely in a sandboxed environment for data analysis and computations';
  ...
  async execute(input: {
    ...
    try {
      ...
      if (this.sandboxed && this.endpoint) {
        // Use external sandboxed Python executor
        result = await this.executeSandboxed(code, packages, inputs);
      } else {
        // Local execution (not recommended for production)
  ...
  private async executeSandboxed(
    ...
    try {
      const response = await axios.post(
        ...
        {
          timeout: this.timeout + 5000, // Add buffer for network
          headers: {
  ...
  private async executeLocal(
    ...
    try {
      ...
      // 1. Use a proper Python subprocess with restricted permissions
      // 2. Set up a virtual environment
      // 3. Install required packages safely
...
# In production, use a proper sandboxed Python executor.
  ...
  private validateCode(code: string): boolean {
    // Basic code validation to prevent obviously dangerous operations
  ...
  async executeDataAnalysis(data: any[], analysisType: string): Promise<PythonExecutionResult> {
...
print(df.head())
  ...
  async executeMathCalculation(expression: string, variables: Record<string, number> = {}): Promise<PythonExecutionResult> {
    // Generate safe math calculation code
...
try:
  ...
  async generateChart(data: any[], chartType: string, options: Record<string, any> = {}): Promise<PythonExecutionResult> {
    const code = `
...
print(f"Chart data (base64): {chart_base64[:100]}...")
  ...
  async validatePythonSyntax(code: string): Promise<boolean> {
